...
func Test_new_registration_should_create_produce_registration_wrapper_msg(t *testing.T) {
	// given
	r := commons.Registration{Ip: "http://my-addr:1234", PlayerId: "id"}
	body, _ := json.Marshal(r)
	mux := http.NewServeMux()
	srv := NewRegistrationServer(mux)
	...
	wr, err := waitRegistrationTimeout(srv.RegChan, time.Second*5)
	assert.Nil(t, err)
...
func Test_new_registration_should_return_500_when_no_answer_from_registration_core(t *testing.T) {
	// given
	r := commons.Registration{Ip: "http://my-addr:1234", PlayerId: "id"}
	body, _ := json.Marshal(r)
	mux := http.NewServeMux()
...
func Test_new_registration_should_return_403_when_refusal_from_registration_core(t *testing.T) {
	// given
	r := commons.Registration{Ip: "http://my-addr:1234", PlayerId: "id"}
	body, _ := json.Marshal(r)
	mux := http.NewServeMux()
